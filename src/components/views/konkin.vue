...
var host = 'http://10.199.14.46:8015/'
    ...
    load() {
      ...
      axios.get(host + 'api/konkin/special/' + this.iduser).then(res => {
        ...
        for (i = 0; i < res.data.length; i++) {
          res.data[i].capaian = Math.random()
          res.data[i].capaian = res.data[i].capaian + '(' + res.data[i].capaian / res.data[i].target_ * 100 + '%)'
        }
        console.log(res.data)
        var jexcelOptions = {
          data: res.data,
