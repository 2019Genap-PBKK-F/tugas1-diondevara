...
export default {
  ...
  methods: {
    add () {
      axios.post('http://10.199.14.46:8015/api/kategoriunit/').then(res => {
        console.log('adding data in new row')
      })
    },
    update (instance, cell, columns, row, value) {
      axios.get('http://10.199.14.46:8015/api/kategoriunit/').then(res => {
        var index = Object.values(res.data[row])
        index[columns] = value
        console.log(index)
        axios.put('http://10.199.14.46:8015/api/kategoriunit/', {
          id: index[0],
          nama: index[1]
        })
      })
    ...
    delete (instance, row) {
      axios.get('http://10.199.14.46:8015/api/kategoriunit/').then(res => {
        var index = Object.values(res.data[row])
        axios.delete('http://10.199.14.46:8015/api/kategoriunit/' + index[0])
  ...
  computed: {
    jexcelOptions () {
      return {
        allowToolbar: true,
        url: 'http://10.199.14.46:8015/api/kategoriunit/',
