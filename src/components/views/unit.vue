...
export default {
  ...
  methods: {
    add () {
      axios.post('http://localhost:8015/api/unit/').then(res => {
        console.log('adding data in new row')
      })
    },
    update (instance, cell, columns, row, value) {
      axios.get('http://localhost:8015/api/unit/').then(res => {
        var index = Object.values(res.data[row])
        index[columns] = value
        console.log(index)
        axios.put('http://localhost:8015/api/unit/', {
          id: index[0],
    ...
    delete (instance, row) {
      axios.get('http://localhost:8015/api/unit/').then(res => {
        var index = Object.values(res.data[row])
        axios.delete('http://localhost:8015/api/unit/' + index[0])
  ...
  computed: {
    jexcelOptions () {
      return {
        allowToolbar: true,
        url: 'http://localhost:8015/api/unit/',
